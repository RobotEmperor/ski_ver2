...
void UpperBodyModule::currentFlagPositionFunction(double x, double y, double z)
{
	...
	head_point_kinematics_->TransformationWaistToHead(0,0,0.352,
			...
			filter_head->lowPassFilter(temp_head_yaw, temp_pre_yaw, 0.01, 0.008));
		...
		std::map<std::string, double> sensors)
{
	...
	result_[joint_id_to_name_[23]]-> goal_position_  = - filter_head->lowPassFilter(temp_head_yaw, temp_pre_yaw, 0.01, 0.008);
	result_[joint_id_to_name_[24]]-> goal_position_  = - filter_head->lowPassFilter(temp_head_pitch, temp_pre_pitch, 0.01, 0.008);
	result_[joint_id_to_name_[25]]-> goal_position_  = - filter_head->lowPassFilter(temp_head_roll, temp_pre_roll, 0.01, 0.008);
